...
응답 규칙(오직 JSON만):
1. "is_matched": 검색된 상품이 구글 시트 요청 품목과 '동일한 핵심 제품'인지 엄격히 검증 (전혀 다른 제품이거나 엉뚱한 브랜드면 false)
...
"""
    ...
    try:
        ...
        if is_matched and crawled_g > 0 and target_g > 0:
            ...
            print(f"  🤖 [Gemini 검증] 일치 여부={is_matched} | 사유={reason}", flush=True)
...
def fetch_danawa_price(product_name, spec):
    ...
    try:
        response = requests.get(search_url, headers=get_headers(), impersonate="chrome120", timeout=10)
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            products = soup.select("li.prod_item:not(.product-pot)")
            
            # 상위 3개 상품을 순차적으로 검증하여 일치하는 상품 검색
            for first_prod in products[:3]:
...
def fetch_direct_url_price(target_url, product_name, spec):
    try:
        response = requests.get(target_url, headers=get_headers(), impersonate="chrome120", timeout=12)
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            
            title_el = soup.select_one("h3.prod_tit") or soup.select_one("h2.prod_title") or soup.select_one("h1")
            title_text = title_el.text.strip() if title_el else product_name

            price_el = soup.select_one("span.prc_c") or soup.select_one("strong.price-value") or soup.select_one("span.price_num") or soup.select_one("em.num")
            raw_price = 0
            if price_el:
                nums = re.findall(r'[\d,]+', price_el.text)
                if nums: raw_price = int(nums[0].replace(",", ""))

            if raw_price == 0:
                matches = re.findall(r'(?:판매가|최저가|가격)\s*:?\s*([\d,]+)\s*원', response.text)
                if matches: raw_price = int(matches[0].replace(",", ""))
            ...
            if raw_price > 0:
                calculated_price, is_matched, ai_shipping = analyze_product_with_gemini(product_name, spec, title_text, raw_price, ship_text)
                channel_name = "지정URL"
                if "danawa" in target_url: channel_name = "다나와(지정)"
                elif "coupang" in target_url: channel_name = "쿠팡(지정)"
                elif "naver" in target_url: channel_name = "네이버(지정)"
                elif "baemin" in target_url: channel_name = "배민상회(지정)"
...
def run_price_update():
    try:
        ...
        if not token_secret: raise ValueError("❌ GCP_TOKEN_JSON이 없습니다.")
        ...
        print("==================================================", flush=True)
        print(f"🚀 [검색어 정제 및 상위 3개 상품 순차 검증 가동] 총 {total_rows}개 행 수집 시작", flush=True)
        print("==================================================\n", flush=True)
        ...
        for row_idx in range(3, total_rows + 1):
            ...
            custom_url = row_values[19].strip() if len(row_values) >= 20 else ""
            ...
            if custom_url.startswith("http"):
                ...
                if valid_results:
                    ...
                    print(f"  🏆 [최저가 확정] 채널: {best_channel} | 환산 최저가: {best_price:,}원", flush=True)
                else:
                    best_channel, best_price, best_shipping, best_link = "검색결과없음", 0, "", "-"
                    print(f"  ⚠️ [검색 결과 없음] 일치하는 상품을 찾지 못했습니다.", flush=True)
        ...
        print("🎉 일치율 최적화 수집 작업이 성공적으로 완료되었습니다!", flush=True)
